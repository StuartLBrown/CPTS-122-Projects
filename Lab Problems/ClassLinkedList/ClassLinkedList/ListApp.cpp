...
void ListApp::runApp(void)
{
	...
	if (mInputStream.is_open()) // verify file is in fact open!
	{
		...
		writeResults(computeMean(), computeMin(), computeMax());
	}
}

double ListApp::computeMean(void) {
	double sum = 0;
	int count = 0;
	List temp = this->mList;
	while (!temp.isEmpty()) {
		count++;
		sum += temp.deleteAtFront();
	}
	return sum / count;
}
int ListApp::computeMin(void) {
	int min = INT16_MAX;
	List temp = this->mList;
	while (!temp.isEmpty()) {
		if (temp.getHeadPtr()->getData() < min)
			min = temp.deleteAtFront();
		else
			temp.deleteAtFront();
	}
	return min;
}
int ListApp::computeMax(void) {
	int max = INT16_MIN;
	List temp = this->mList;
	while (!temp.isEmpty()) {
		if (temp.getHeadPtr()->getData() > max)
			max = temp.deleteAtFront();
		else
			temp.deleteAtFront();
	}
	return max;
...
void ListApp::writeResults(double mean, int min, int max) {
	mOutputStream << "Mean: " << mean << "\nMin: " << min << "\nMax: " << max << std::endl;
}
